		...
		<div className="bg-white dark:bg-slate-900 w-full max-w-2xl rounded-2xl shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
			<div className="p-4 border-b border-slate-100 dark:border-slate-800 flex justify-between items-center bg-slate-50 dark:bg-slate-800/50">
				<h2 className="text-lg font-bold text-slate-800 dark:text-white flex items-center gap-2">
					<Info size={20} className="text-blue-500" /> Hilfe & Anleitung
				</h2>
				<button
					onClick={onClose}
			...
			<div className="p-6 overflow-y-auto space-y-6 text-sm text-slate-600 dark:text-slate-300">
				<section>
					...
					<p>
						Willkommen im Drachenboot Planer! Diese App hilft dir, dein
						Team zu verwalten, Trainings zu planen und die optimale
						Bootsbesetzung zu berechnen.
					</p>
				</section>
				<section>
					<h3 className="font-bold text-slate-900 dark:text-white text-base mb-2 flex items-center gap-2">
					...
					<ul className="list-disc pl-5 space-y-1">
						...
						<li>
							...
							"Trommel"/"Steuer"-Skills werden vom Auto-Fill auf diese
							Plätze gesetzt.
						</li>
					</ul>
				</section>
			...
			for (let i = 0; i < 1500; i++) {
				...
				['row-1-left', 'row-1-right'].forEach((sid) => {
					...
					const cands = light.filter(
						(p) =>
							currPool.includes(p) && p.skills && p.skills.includes(side)
					);
					if (cands.length) {
				...
				currPool.sort(
					(a, b) =>
						b.weight + Math.random() * 5 - (a.weight + Math.random() * 5)
				);
				const free = [];
				...
				for (let p of currPool) {
					...
					Object.entries(currAss).forEach(([sid, pid]) => {
						...
						if (parseInt(sid.match(/row-(\d+)/)[1]) <= 5) f += pad.weight;
						else b += pad.weight;
					});
					const nLeft = l <= r,
				...
				Object.entries(currAss).forEach(([sid, pid]) => {
					...
					if (parseInt(sid.match(/row-(\d+)/)[1]) <= 5) ff += pad.weight;
					else fb += pad.weight;
				});
						...
						<div className="lg:col-span-1 space-y-4">
							...
							<div className="space-y-4">
								{events
									...
									.map((evt) => {
										const yesCount = Object.values(evt.attendance).filter(
											(s) => s === 'yes'
										).length;
										return (
											<div
														...
														return (
															...
															>
																...
																<div className="flex gap-1">
																	<button
																		onClick={() =>
																			updateAttendance(evt.id, p.id, 'yes')
																		}
																	...
																	<button
																		onClick={() =>
																			updateAttendance(evt.id, p.id, 'no')
																		}
																		className={`w-8 h-8 flex items-center justify-center rounded-lg border transition-all ${
																			status === 'no'
							...
							>
								...
								<form onSubmit={handleSavePaddler} className="space-y-4">
									...
									<div className="flex gap-2 pt-2">
										...
										<button
											type="submit"
											className={`text-white px-6 py-2 rounded text-sm font-medium flex items-center gap-2 ${
												editingPaddlerId
										...
										>
											...
											{editingPaddlerId ? 'Speichern' : 'Hinzufügen'}
										</button>
									</div>
								</form>
							</div>
						...
						<div className="bg-white dark:bg-slate-900 p-4 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800">
							...
							<div className="mt-4 pt-4 border-t border-slate-100 dark:border-slate-800">
								...
								<input
									...
									step="5"
									value={targetTrim}
									onChange={(e) => setTargetTrim(parseInt(e.target.value))}
							...
							<div className="grid grid-cols-3 gap-2 mt-4">
								<button
									onClick={runAutoFill}
									disabled={isSimulating || activePaddlerPool.length === 0}
						...
						<div className="bg-white dark:bg-slate-900 p-4 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 h-[600px] flex flex-col">
							...
							<div className="flex-1 overflow-y-auto pr-1 space-y-2">
								{activePaddlerPool.map((p) => {
									const isAssigned = Object.values(assignments).includes(
										p.id
									);
											...
											{(() => {
												...
												return (
													<SeatBox
														...
														onUnassign={(e) => handleUnassign(s.id, e)}
