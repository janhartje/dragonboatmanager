          ...
          <div className="lg:col-span-1 lg:order-1 flex flex-col">
            <div className="bg-white dark:bg-slate-900 p-6 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 h-full">
              ...
              <form onSubmit={handleCreateEvent} className="space-y-4">
                <div>
                  <label className="text-[10px] uppercase font-bold text-slate-500 dark:text-slate-400 mb-1 block">Titel</label>
                  <input className="w-full p-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded text-sm outline-none text-slate-800 dark:text-white placeholder:text-slate-400" placeholder="Training / Regatta" value={newEventTitle} onChange={(e) => setNewEventTitle(e.target.value)} />
                </div>
                <div>
                  <label className="text-[10px] uppercase font-bold text-slate-500 dark:text-slate-400 mb-1 block">Datum</label>
              ...
              <form onSubmit={handleSavePaddler} className="space-y-4">
                <div className="flex flex-col md:flex-row gap-4">
                  <div className="flex-1">
                    <label className="text-[10px] uppercase font-bold text-slate-500 dark:text-slate-400 mb-1 block">Name</label>
                    <input className="w-full p-2 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded text-sm outline-none dark:text-white" value={paddlerFormName} onChange={(e) => setPaddlerFormName(e.target.value)} placeholder="Name" />
                  </div>
                  <div className="w-full md:w-32">
                    <label className="text-[10px] uppercase font-bold text-slate-500 dark:text-slate-400 mb-1 block">Gewicht</label>
                ...
                <div>
                  <label className="text-[10px] uppercase font-bold text-slate-500 dark:text-slate-400 mb-2 block">Skills</label>
                  <div className="flex gap-2 flex-wrap">
                    <button type="button" onClick={() => toggleSkill('left')} className={`px-3 py-1.5 rounded-lg border text-sm font-medium transition-all ${paddlerFormSkills.left ? 'bg-red-500 border-red-600 text-white' : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400'}`}>Links</button>
                    <button type="button" onClick={() => toggleSkill('right')} className={`px-3 py-1.5 rounded-lg border text-sm font-medium transition-all ${paddlerFormSkills.right ? 'bg-green-500 border-green-600 text-white' : 'bg-white dark:bg-slate-800 border-slate-200 dark:border-slate-700 text-slate-500 dark:text-slate-400'}`}>Rechts</button>
                ...
                <div className="flex gap-2 pt-2">
                  ...
                  <button type="submit" className={`text-white h-9 px-6 py-2 rounded text-sm font-medium flex items-center gap-2 ${editingPaddlerId ? 'bg-orange-500 hover:bg-orange-600' : 'bg-blue-600 hover:bg-blue-700'}`}>
                    {editingPaddlerId ? <Save size={16} /> : <Plus size={16} />} {editingPaddlerId ? 'Speichern' : 'Hinzufügen'}
                  </button>
                </div>
              </form>
            </div>
                  ...
                  <div key={evt.id} className="bg-white dark:bg-slate-900 p-4 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800 hover:shadow-md transition-shadow">
                    <div className="flex justify-between items-start mb-2">
                      ...
                      <div className={`text-[10px] px-2 py-1 rounded-full font-bold uppercase ${evt.type === 'race' ? 'bg-purple-100 dark:bg-purple-900 text-purple-700 dark:text-purple-200' : 'bg-blue-50 dark:bg-blue-900 text-blue-600 dark:text-blue-200'}`}>
                        {evt.type === 'race' ? 'Regatta' : 'Training'}
                      </div>
                    </div>
                    <div className="flex justify-between items-center mt-4 pb-4 border-b border-slate-100 dark:border-slate-800">
                      <div className="text-sm font-medium text-slate-600 dark:text-slate-300 bg-slate-100 dark:bg-slate-800 px-2 py-1 rounded-lg"><span className="text-green-600 dark:text-green-400 font-bold">{yesCount}</span> Zusagen</div>
                      <button onClick={() => handlePlanEvent(evt.id)} className="text-sm bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 flex items-center gap-1">Planen <ChevronRight size={16} /></button>
                    </div>
