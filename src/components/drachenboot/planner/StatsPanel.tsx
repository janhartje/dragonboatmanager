  ...
  return (
    <div id="tour-planner-stats" className="bg-white dark:bg-slate-900 p-4 rounded-xl shadow-sm border border-slate-200 dark:border-slate-800">
      <div className="flex items-center justify-between mb-3">
        ...
        <div id="tour-planner-boat-size" className="flex bg-slate-100 dark:bg-slate-800 rounded-lg p-1">
          <button 
            ...
            className={`px-2 py-1 text-xs font-bold rounded-md transition-all ${boatSize === 'standard' ? 'bg-white dark:bg-slate-700 text-blue-600 dark:text-blue-400 shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-300'} ${isReadOnly ? 'opacity-50 cursor-not-allowed' : ''}`}
          >
            {t('standardBoat')}
          </button>
          <button 
            ...
            className={`px-2 py-1 text-xs font-bold rounded-md transition-all ${boatSize === 'small' ? 'bg-white dark:bg-slate-700 text-blue-600 dark:text-blue-400 shadow-sm' : 'text-slate-500 dark:text-slate-400 hover:text-slate-700 dark:hover:text-slate-300'} ${isReadOnly ? 'opacity-50 cursor-not-allowed' : ''}`}
          >
            {t('smallBoat')}
          </button>
        </div>
      ...
      <div className="mt-4 pt-4 border-t border-slate-100 dark:border-slate-800">
        ...
        <input disabled={isReadOnly} type="range" min="-100" max="100" step="5" value={targetTrim} onChange={(e) => setTargetTrim(parseInt(e.target.value))} className={`w-full h-2 bg-slate-200 dark:bg-slate-700 rounded-lg appearance-none cursor-pointer accent-blue-600 ${isReadOnly ? 'opacity-50 cursor-not-allowed' : ''}`} />
        <div className="flex justify-between text-[10px] text-slate-400 mt-1"><span>-100kg</span><span>0</span><span>+100kg</span></div>
      </div>

      <div className="grid grid-cols-3 gap-2 mt-4">
        <button id="tour-planner-autofill" onClick={runAutoFill} disabled={isSimulating || activePaddlerPool.length === 0 || isReadOnly} className={`py-3 text-sm font-semibold rounded-lg border dark:border-slate-700 transition-all flex items-center justify-center gap-2 shadow-sm ${isSimulating ? 'bg-indigo-50 text-indigo-400 cursor-wait' : 'bg-indigo-600 text-white hover:bg-indigo-700 active:scale-95'} ${activePaddlerPool.length === 0 || isReadOnly ? 'opacity-50 cursor-not-allowed' : ''}`} title={t('magicAI')}>
        ...
        <button disabled={isReadOnly} onClick={clearBoat} className={`py-3 text-sm rounded-lg border dark:border-slate-700 transition-all flex items-center justify-center gap-2 active:scale-95 ${confirmClear ? 'bg-red-500 text-white border-red-600 font-bold' : 'text-red-500 hover:bg-red-50 dark:hover:bg-red-900/20 border-red-200'} ${isReadOnly ? 'opacity-50 cursor-not-allowed' : ''}`}>
